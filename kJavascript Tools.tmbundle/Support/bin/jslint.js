...
function countMember(m){if(membersOnly&&typeof membersOnly[m]!=='boolean'){warning("Unexpected /*member '{a}'.",nexttoken,m);}
...
for(;;){if(nexttoken.identifier){if(!is_own(htmltag,nexttoken.value)){warning("Expected a tagName, and instead saw {a}.",nexttoken,nexttoken.value);}
...
return JSLINT.errors.length===0;};function to_array(o){var a=[],k;for(k in o){if(is_own(o,k)){a.push(k);}}
return a;}
...
for(i=1;i<functions.length;i+=1){f=functions[i];cl=[];ex=[];va=[];un=[];ou=[];gl=[];la=[];for(k in f){if(is_own(f,k)&&k.charAt(0)!=='('){v=f[k];switch(v){case'closure':cl.push(k);break;case'var':va.push(k);break;case'exception':ex.push(k);break;case'outer':ou.push(k);break;case'global':gl.push(k);break;case'label':la.push(k);break;case'unused':un.push(k);break;}}}
o.push('<br><div class=function><i>'+f['(line)']+'</i> '+
(f['(name)']||'')+'('+
(f['(params)']||'')+')</div>');detail('Closure',cl);detail('Variable',va);detail('Exception',ex);detail('Outer',ou);detail('Global',gl);detail('Label',la);detail('<big><b>Unused</b></big>',un);}
...
o.push(m+'<br>*/</pre>');}
o.push('</div>');}
return o.join('');};itself.edition='2009-07-08';return itself;}());(function(a){if(!a[0]){print("Usage: jslint.js file.js");quit(1);}
